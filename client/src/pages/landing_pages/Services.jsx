...
const Services = () => {
  ...
  return (
    <div
      className={`min-h-screen font-sans antialiased transition-colors duration-300 ${theme === "dark" ? "bg-[#0f172a]" : "bg-white"}`}
    >
      <Navbar />

      {/* --- Hero Section --- */}
      <section
        className={`relative pt-32 pb-24 flex items-center overflow-hidden transition-all duration-500 ${
          theme === "dark"
      ...
      >
        ...
        <div className="absolute top-0 right-0 w-125 h-125 bg-blue-500/10 blur-[120px] rounded-full -mr-64 -mt-64"></div>
        <div className="absolute bottom-0 left-0 w-75 h-75 bg-blue-400/10 blur-[100px] rounded-full -ml-32 -mb-32"></div>

        <div className="w-full max-w-300 mx-auto px-6 relative z-10">
          <div className="max-w-2xl">
            <div
              className={`inline-flex items-center gap-2 px-3 py-1 rounded-full border mb-8 backdrop-blur-md ${
                theme === "dark"
            ...
            <div className="flex flex-wrap items-center gap-5">
              <Link
                to="/cars"
                className="bg-white text-blue-600 hover:bg-blue-50 px-8 py-4 rounded-2xl font-bold transition-all flex items-center gap-2 text-sm shadow-xl shadow-blue-900/20"
              >
                EXPLORE FLEET <ChevronRight size={18} />
              </Link>
              <Link
                to="/contacts"
                className="bg-blue-500/30 hover:bg-blue-500/50 text-white border border-white/30 backdrop-blur-md px-8 py-4 rounded-2xl font-bold transition-all text-sm"
              >
                CONTACT US
              </Link>
            </div>
      ...
      {/* --- Services Grid --- */}
      <section
        className={`py-24 transition-colors duration-300 ${theme === "dark" ? "bg-[#0f172a]" : "bg-blue-50"}`}
      >
        <div className="w-full max-w-300 mx-auto px-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
            {services.map((item, idx) => (
              <div
                key={idx}
                className={`group relative p-10 border transition-all duration-500 rounded-4xl shadow-xl hover:-translate-y-2 ${
                  theme === "dark"
                    ? "bg-slate-900 border-slate-800 hover:bg-blue-600 shadow-black/20"
                    : "bg-white border-blue-100 hover:bg-blue-600 shadow-blue-900/5"
                }`}
              >
                <div
                ...
                <h4
                  className={`text-xl font-bold mb-4 group-hover:text-white transition-colors ${theme === "dark" ? "text-white" : "text-slate-900"}`}
                >
                  {item.title}
                </h4>
                <p
                  className={`text-sm leading-relaxed mb-8 group-hover:text-blue-100 transition-colors ${theme === "dark" ? "text-slate-400" : "text-slate-500"}`}
                >
                  {item.desc}
                </p>
                ...
                <Link
                  to="/contacts"
                  className={`text-xs font-bold uppercase tracking-widest flex items-center gap-2 transition-colors group-hover:text-white ${theme === "dark" ? "text-blue-400" : "text-blue-600"}`}
                >
      ...
      {/* --- Footer --- */}
      <footer
        className={`transition-colors duration-300 pt-24 pb-12 ${theme === "dark" ? "bg-slate-950 border-t border-slate-900" : "bg-blue-900"}`}
      >
        <div className="w-full max-w-300 mx-auto px-6 text-blue-100">
            ...
            <div className="col-span-1">
              ...
              <div className="flex gap-4">
                {[Facebook, Twitter, Instagram].map((Icon, i) => (
                  <div
                    key={i}
                    className={`w-10 h-10 rounded-xl flex items-center justify-center hover:bg-white hover:text-blue-900 transition-all cursor-pointer ${
